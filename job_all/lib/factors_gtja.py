...
class Alphas(object):
    ...
    def alpha007(self):
        data_mid1=((self.high+self.low)/2-(delay(self.high,1)+delay(self.low,1))/2)*(self.high-self.low)/self.volume
        return data_mid1.ewm(alpha=2/7).mean()
    ...
    def alpha024(self):
        ...
        return data_mid.ewm(alpha=1/5).mean()

    def alpha028(self):
        data_mid1=((self.close-ts_min(self.low,9)/(ts_max(self.high,9)-ts_min(self.low,9))*100)).ewm(alpha=1/3).mean()
        data_mid2=((self.close-ts_min(self.low,9))/(ts_max(self.high,9)-ts_max(self.low,9))*100).ewm(alpha=1/3).mean()
        data_mid3=data_mid2.ewm(alpha=1/3).mean()
    ...
    def alpha047(self):
        ...
        return 100*(data_mid1/data_mid2).ewm(alpha=1/9).mean()
    ...
    def alpha057(self):
        ...
        return data_mid1.ewm(alpha=1/3).mean()
    ...
    def alpha063(self):
        ...
        return ((data_mid1.ewm(alpha=1/6).mean())/(data_mid2.ewm(alpha=1/6).mean()))*100
    ...
    def alpha067(self):
        ...
        data_mid1 = pd.Series(data_mid1, name="values")
        data_mid2 = (self.close - delay(self.close)).abs()
        return ((data_mid1.ewm(alpha=1 / 24).mean()) / (data_mid2.ewm(alpha=1 / 24).mean())) * 100

    def alpha068(self):
        data_mid1=((self.high+self.low)/2-(delay(self.high)+delay(self.low))/2)*(self.high-self.low)/self.volume
        return data_mid1.ewm(alpha=2/15).mean()
    ...
    def alpha072(self):
        ...
        return data_mid1.ewm(alpha=1/15).mean()
    ...
    def alpha079(self):
        ...
        data_mid1[data_mid1<0]=0
        return ((data_mid1).ewm(alpha=1/12).mean())/(((self.close-delay(self.close)).abs()).ewm(alpha=1/12).mean())*100

    def alpha080(self):
        return (self.volume-delay(self.volume,5))/delay(self.volume,5)*100

    def alpha081(self):
        return self.volume.ewm(alpha=2/21).mean()/self.volume

    def alpha082(self):
        data_mid1=ts_max(self.high,6)-self.close
        data_mid2=ts_max(self.high,6)-ts_min(self.low,6)
        return (100*(data_mid1/data_mid2)).ewm(alpha=1/20).mean()
    ...
    def alpha089(self):
        data_mid1=self.close.ewm(alpha=2/13).mean()-self.close.ewm(alpha=2/27).mean()-(self.close.ewm(alpha=2/13).mean()-self.close.ewm(alpha=2/27).mean()).ewm(alpha=2/10).mean()
        return data_mid1*2
    ...
    def alpha096(self):
        return (((self.close-ts_min(self.low,9))/(ts_max(self.high,9)-ts_min(self.low,9))*100).ewm(alpha=1/3).mean()).ewm(alpha=1/3).mean()
    ...
    def alpha102(self):
        ...
        return (data_mid.ewm(alpha=1/6).mean())/(((self.volume-delay(self.volume)).abs()).ewm(alpha=1/6).mean())*100
    ...
    def alpha109(self):
        return (self.high-self.low).ewm(alpha=2/10).mean()/((self.high-self.low).ewm(alpha=2/10).mean()).ewm(alpha=2/10).mean()
    ...
    def alpha111(self):
        return (((2*self.close-self.low-self.high)/(self.high-self.low))*self.volume).ewm(alpha=2/11).mean()-(((2*self.close-self.low-self.high)/(self.high-self.low))*self.volume).ewm(alpha=2/4).mean()
    ...
    def alpha122(self):
        return ((((self.close.map(np.log)).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()-delay((((self.close.map(np.log)).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()))/(delay((((self.close.map(np.log)).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()))
    ...
    def alpha135(self):
        return delay(self.close/delay(self.close,20)).ewm(alpha=1/20).mean()
    ...
    def alpha146(self):
        data_mid1=sma(((self.close-delay(self.close))/delay(self.close)-((self.close-delay(self.close))/delay(self.close)).ewm(alpha=2/61).mean()),20)
        data_mid2=(self.close-delay(self.close))/delay(self.close)-((self.close-delay(self.close))/delay(self.close)).ewm(alpha=2/61).mean()
        data_mid3=(((self.close-delay(self.close))/delay(self.close)-((self.close-delay(self.close))/delay(self.close)-((self.close-delay(self.close))/delay(self.close)-((self.close-delay(self.close))/delay(self.close)).ewm(alpha=2/61).mean())))**2).ewm(alpha=2/61).mean()
    ...
    def alpha151(self):
        return (self.close-delay(self.close,20)).ewm(alpha=1/20).mean()

    def alpha152(self):
        data_mid1=sma((delay((delay(self.close/delay(self.close,9))).ewm(alpha=1/9).mean())),12)
        data_mid2=sma((delay((delay(self.close/delay(self.close,9))).ewm(alpha=1/9).mean())),26)
        return (data_mid1-data_mid2).ewm(alpha=1/9).mean()

    def alpha153(self):
        return (sma(self.close,3)+sma(self.close,6)+sma(self.close,12)+sma(self.close,24))/(4*self.close)

    def alpha155(self):
        return self.volume.ewm(alpha=2/13).mean()-self.volume.ewm(alpha=2/27).mean()-((self.volume).ewm(alpha=2/13).mean()-(self.volume).ewm(alpha=2/27).mean()).ewm(alpha=2/10).mean()

    def alpha158(self):
        return ((self.high-self.close.ewm(alpha=2/15).mean())-(self.low-self.close.ewm(alpha=2/15).mean()))/self.close
    ...
    def alpha160(self):
        ...
        return data_mid.ewm(alpha=1/20).mean()/(self.close)
    ...
    def alpha162(self):
        data_mid1=self.close-delay(self.close)
        data_mid1[data_mid1<0]=0
        data_mid2=data_mid1.ewm(alpha=1/12).mean()/(self.close-delay(self.close)).ewm(alpha=1/12).mean()*100
    ...
    def alpha164(self):
        ...
        return ((data_mid1-data_mid2)/(self.high-self.low)*100).ewm(alpha=2/13).mean()
    ...
    def alpha169(self):
        data_mid=delay((self.close-delay(self.close)).ewm(alpha=1/9).mean())
        return (sma(data_mid,12)-sma(data_mid,26)).ewm(alpha=1/10).mean()
    ...
    def alpha173(self):
        return 3*(self.close.ewm(alpha=2/13).mean())-2*(self.close.ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()+(((log(self.close)).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()).ewm(alpha=2/13).mean()/self.close

    def alpha174(self):
        data_mid1=stddev(self.close,20)
        data_mid1[self.close<=delay(self.close)]
        return (data_mid1).ewm(alpha=1/20).mean()/self.close
    ...
    def alpha188(self):
        return ((self.high-self.low-(self.high-self.low).ewm(alpha=2/11).mean())/((self.high-self.low).ewm(alpha=2/11).mean()))*100
