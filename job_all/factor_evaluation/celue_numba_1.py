...
# print(len(data_zb))
data_k = data_k[["tickid", "open", "high", "close", "low", "growth", "high_35", "ma7", "ma90", "low_30", "low_days"]]
        ...
        if (len(data_k[data_k[:, 0] < data_zb[n][0]]) < 35) or (n % 6 != 0):
            ...
            if btcnum_list[n-1] == 0.0:
                if data_zb[n][1] < data_k[data_k[:, 0] < data_zb[n][0]][-1][9]:
                    tupo_l = 1
                    tupo = 0
                if (data_zb[n][1] > data_k[data_k[:, 0] < data_zb[n][0]][-1][6]) and (tupo_l == 1) and (data_k[data_k[:, 0] < data_zb[n][0]][-1][10] < 7):
                    tupo = 1
                    tupo_l = 0
                if (tupo == 1) and \
                        (data_k[data_k[:, 0] < data_zb[n][0]][-1][8] >= data_k[data_k[:, 0] < data_zb[n][0]][-2][8]) and \
