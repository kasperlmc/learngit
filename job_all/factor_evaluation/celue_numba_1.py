...
data_k["high_35"] = ts_max(data_k["high"], window=35)
...
# print(len(data_zb))
data_k = data_k[["tickid", "open", "high", "close", "low", "atr", "growth", "high_35"]]
        ...
        if len(data_k[data_k[:, 0] < data_zb[n][0]]) < 20:
            ...
            if btcnum_list[n-1] == 0.0:
                if (data_zb[n][1] > data_k[data_k[:, 0] < data_zb[n][0]][-1][7]) and \
                        ...
                        (data_k[data_k[:, 0] < data_zb[n][0]][-1][6] < 4):
                    print("开仓买入")
                    print(data_zb[n][0])
                ...
                if data_zb[n][1] < min(data_zb[n-840:n][:, 1].max()-data_k[data_k[:, 0] < data_zb[n][0]][-1][5]*2, data_zb[n-840:n][:, 1].max()*0.995):
                    print("平仓卖出")
                    print(data_zb[n][0])
