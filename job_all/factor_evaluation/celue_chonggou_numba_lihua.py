...
# 数据准备，导入币安各个币对
coinpair="ethusdt"
data_zb = pd.read_csv("/Users/wuyong/alldata/original_data/trades_BIAN_"+coinpair+"_s_3mon.csv", index_col=0)
data_zb["tickid"] = data_zb["dealtime"]
data_zb["close_s"] = data_zb["price"]
data_k = pd.read_csv("/Users/wuyong/alldata/original_data/trades_bian_"+coinpair+"_m_3mon.csv", index_col=0)
# 前期各个指标的准备
...
data_k["ma7"] = data_k["close"].rolling(window=7,center=False).mean()
data_k["ma30"] = data_k["close"].rolling(window=30,center=False).mean()
data_k["ma90"] = data_k["close"].rolling(window=90,center=False).mean()
data_k["ma450"] = data_k["close"].rolling(window=450,center=False).mean()
...
data_k["last_10_high45"] = ts_max(data_k["high"], window=45).shift(10)


# data_k["close_8"] = data_k["close"].shift(7)
data_zb = data_zb[["tickid", "close_s"]]
data_k = data_k[["tickid", "open", "high", "close", "low", "high_55", \
                 "ma7", "ma90", "low_55", "ma450", "ma30", "ma90_diff", "low_25", "ma90_div", "ou_func", "ou_ma90", "low_days", "last_10_high45"]]
...
data_combine["tupo_up"] = [1 if (x > y) and (y == z) else 0 for x, y, z in zip(data_combine["close_s"].values, data_combine["high_55"].shift(1).values, data_combine["last_10_high45"].shift(1).values)]
data_combine["shuaxin"] = [1 if x == y else 0 for x, y in zip(data_combine["high_55"].values, data_combine["last_10_high45"].shift(1).values)]
#data_combine["buy"] = [1 if (x > y) and (z < 6) and (u == 1) and (v >= 0.9999) and (w == 1) and ((x-a)/a > 0.015 and True) else 0 for x, y, z, v, w, u, a \
data_combine["buy"] = [1 if (x > 1.01*y) and (v >= 0.9999) else 0 for x, y, z, v, w7, u90, a, ma450 \
            in zip(data_combine["close_s"].values, data_combine["last_10_high45"].shift(1).values, data_combine["low_days"].shift(1).values, \
                   data_combine["ma90_div"].shift(1).values,data_combine["ou_func"].shift(1).values, data_combine["ou_ma90"].shift(1).values, data_combine["low_55"].shift(1).values, data_combine["ma450"].shift(1).values)] 
...
data_celue = data_combine[["tupo_up", "shuaxin", "buy", "sell_1", "sell_2", "tickid", "close_s"]]
print(data_celue.head(100))

data_celue = data_celue[data_celue["tickid"] > 1527782400]
print("回测起始时间:%s" %pd.to_datetime(1527782400,unit='s'))
        ...
        if n < 100:
            ...
            if btcnum_list[n-1] == 0.0:
                ...
                if (data_celue[n][2] == 1) and (tupo_l == 1):
                    ...
                    tupo_l = 0
                    print("开仓买入")
                    print("买入时点为%s，买入时点价格为%s" % (pd.to_datetime(data_celue[n][5],unit='s'), data_celue[n][6]))
                ...
                if (data_celue[n][3] == 1) or (data_celue[n][4] == 1):
                    position_list[n] = 0
                    print("平仓卖出")
                    print("卖出时点为%s，卖出时点价格为%s" % (pd.to_datetime(data_celue[n][5],unit='s'), data_celue[n][6]))
...
print(coinpair+"—————————————10——————04—————————————02—————01—————————")
# # print(numba_celue(data_zb.values,data_k.values))
cash_list, asset_list,btcnum_list,tradetimes,profitnum,profittimes,lossnum,losstimes, date_list, position_list \
= numba_celue(data_celue.values)
end = time.time()
...
# df_result.to_csv(coinpair+"_traderesult.csv")
