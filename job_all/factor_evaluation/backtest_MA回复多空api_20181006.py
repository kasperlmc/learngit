...
def summary(net_df, signal_df, param,plot_in_loops):
    ...
    cols = ['factor', 'param', 'tot_ret', 'ann_ret', 'sharpe', 'annualVolatility',
...
if __name__ == '__main__':
    exchange = 'BIAN'
    alphas = ["Alpha.alpha014"]
    params = ["Alpha.alpha014_rank_20"]
    ...
    for factor in alphas:
        df = pd.read_csv('../factor_writedb/btcusdt_'+ factor +'.csv', index_col=0)
        # 回测时间段
        ...
        for param in params:
            ...
            stat, cols = summary(net_df_hv, signal_df_hv, param,0)
            stat_ls.append(stat)
            stat, cols = summary(net_df_lv, signal_df_lv, param,0)
            stat_ls.append(stat)
            stat, cols = summary(net_df, signal_df, param, 1)
            stat_ls.append(stat)
