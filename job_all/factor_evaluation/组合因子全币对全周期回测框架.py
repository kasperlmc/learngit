...
def summary_net(net_df, param, plot_in_loops):
    ...
    result = [param, tot_ret, ann_ret, sharpe, annualVolatility,
    ...
    cols = ["param", 'tot_ret', 'ann_ret', 'sharpe', 'annualVolatility',
...
if __name__ == '__main__':
    ...
    alphas = ["Alpha.alpha009","Alpha.alpha118"]
    params = ["Alpha.alpha009","Alpha.alpha118"]
    corr_list = [1, 1]
    ...
    for symbol in symbols:
        for t in period:
            for i in range(len(alphas)):
                df = pd.read_csv('../factor_writedb/' + symbol + "_" + alphas[i] + "_" + t + '.csv', index_col=0)
            ...
            #print(net_df_all)
            stat, cols = summary_net(net_df_all,"all_"+symbol+t, 0)
