...
# print(len(data_zb))
data_k = data_k[["tickid", "open", "high", "close", "low", "growth", "high_35", "ma7", "ma90", "low_30", "low_days", "ma30", "ma90_diff"]]
...
def numba_celue(data_zb, data_k):
    ...
    for n in range(1, len(data_zb)):
        if (len(data_k[data_k[:, 0] < data_zb[n][0]]) < 35) or (n % 12 != 0):
            ...
            if btcnum_list[n-1] == 0.0:
                ...
                elif (data_zb[n][1] > data_k[data_k[:, 0] < data_zb[n][0]][-1][6]) and (tupo_l == 1):
                    tupo_l = 0
                    cash_list = np.append(cash_list, cash_list[-1])
                    btcnum_list = np.append(btcnum_list, btcnum_list[-1])
                    asset_list = np.append(asset_list, asset_list[-1])
                ...
                if (data_zb[n][1] < data_k[data_k[:, 0] < data_zb[n][0]][-25:][:, 4].min()) or \
                        ((data_zb[n][1] < data_k[data_k[:, 0] < data_zb[n][0]][-1][11]) and (data_k[data_k[:, 0] < data_zb[n][0]][-1][12] < 0)):
                    print("平仓卖出")
