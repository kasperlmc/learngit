...
data=pd.read_csv('xbtusd_'+ "result_df_all_all_4h" +'_stat.csv', index_col=0)
print(len(data))
...
print(df)
df.to_csv("all_xbtusd_factor_eva.csv")
...
for i in range(len(alpha_list)):
    df_data_x = pd.read_csv('../factor_writedb/double_factors' + alpha_list[i] + '.csv', index_col=0)
    factor_x=df_data_x[param_list[i]]
    tem_list=[]
    for v in range(len(alpha_list)):
        df_data_y=pd.read_csv('../factor_writedb/double_factors' + alpha_list[v] + '.csv', index_col=0)
