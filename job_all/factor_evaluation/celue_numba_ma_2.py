...
# 数据准备，数据说明
data_zb = pd.read_csv("/Users/wuyong/alldata/original_data/trades_bian_ethusdt_s_2.csv", index_col=0)
# print(data_zb.head())
...
# print(len(data_zb))
data_k = pd.read_csv("/Users/wuyong/alldata/original_data/trades_bian_ethusdt_m_2.csv", index_col=0)
...
data_k["filter"] = [1 if (x>y) or (x<y*0.95) else 0 for x,y in zip(data_k["ma90"].values,data_k["ma450"].values)]
# data_k["diff"] = data_k["diff"].diff()
# data_k["diff"] = data_k["diff"].apply(lambda x: 1 if x > 0 else x)
# data_k["diff"] = data_k["diff"].apply(lambda x: 0 if x < 0 else x)
data_k["diff"] = ts_sum(data_k["diff"], window=35)
...
data_zb = data_zb[data_zb["tickid"] > 1543680000]  # 将数据减少为周数据
# print(len(data_zb))
data_k = data_k[["tickid", "open", "high", "close", "low", "growth", "high_35", "ma7", "ma90", "low_30", "low_days", "filter"]]
...
def numba_celue(data_zb, data_k):
    ...
    for n in range(1, len(data_zb)):
        if (len(data_k[data_k[:, 0] < data_zb[n][0]]) < 35) or (n % 6 != 0):
            ...
            if btcnum_list[n-1] == 0.0:
                if data_zb[n][1] < data_k[data_k[:, 0] < data_zb[n][0]][-1][9]:
                    tupo_l = 1
                if (data_zb[n][1] > data_k[data_k[:, 0] < data_zb[n][0]][-1][6]) and (tupo_l == 1) and \
                        (data_k[data_k[:, 0] < data_zb[n][0]][-1][10] < 7) and \
                        (data_k[data_k[:, 0] < data_zb[n][0]][-1][11] == 1):
                    tupo_l = 0
                    print("开仓买入")
                ...
                if data_zb[n][1] < data_k[data_k[:, 0] < data_zb[n][0]][-25:][:, 4].min():
                    print("平仓卖出")
