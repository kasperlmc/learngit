...
if __name__ == '__main__':
    ...
    for name in alpha_test:
        try:
            ...
            factor_df = pd.read_csv('../factor_writedb/btcusdt_' + name + '.csv', index_col=0)
            ...
            factors=[name+"_rank_"+str(x) for x in rank_params]
            stat_ls = []
            for factor in factors:
                try:
                    stat, cols = factor_analyse(factor_df, factor)
                    ...
                    pass
            stat_df = pd.DataFrame(stat_ls, columns=cols, index=factors[:len(stat_ls)])
            for x in days:
