...
if __name__ == '__main__':
    exchange = 'BIAN'
    alphas = ["Alpha.alpha009","Alpha.alpha014_gtja"]
    params = ["Alpha.alpha009_rank_10","Alpha.alpha014"]

    stat_ls = []

    df = pd.read_csv('../factor_writedb/btcusdt_' + alphas[0] + '.csv', index_col=0)
    df1=pd.read_csv('../factor_writedb/btcusdt_' + alphas[1] + '.csv', index_col=0)
    # 回测时间段
    start_day = pd.to_datetime('2017-08-18')
    end_day = pd.to_datetime('2018-10-01')

    df = calc_alpha_signal(df, params[0], start_day, end_day, corr=1)
    df1 = calc_alpha_signal(df1, params[1], start_day, end_day, corr=1)

    net_df, signal_df, end_capital = do_backtest(df, params[0], start_day, end_day)
    net_df1, signal_df1, end_capital = do_backtest(df1, params[1], start_day, end_day)
    net_df_all=net_df1[["net","pos","tradecoin","basecoin"]]+net_df[["net","pos","tradecoin","basecoin"]]
    net_df_all[["date_time","close"]]=net_df[["date_time","close"]]
    print(net_df_all.tail())
    stat,cols=summary_net(net_df_all,0)
    print(stat)

    results1, col_signal1=summary_signal(signal_df)
    results2, col_signal2=summary_signal(signal_df1)
    results_all=[(x+y)/2 for x,y in zip(results1,results2)]
