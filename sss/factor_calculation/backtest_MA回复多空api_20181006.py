...
def calc_signal(df, param):
    ...
    # close 回归 lb 则做多
    df['open_long_signal'] = ((df['close'] > df['lb']).shift(1)
                              & (df['close'] < df['lb']).shift(2))*1
    # close 上穿 MA 则平仓
...
def summary(net_df, signal_df, param):
    ...
    result = (symbol, param, tot_ret, ann_ret, sharpe,
              drawdown, alpha, beta, ret_r, ir, hold, pos, trad_time, win_r,
              profit_r, long_ret, short_ret,
              net_df['date_time'].iloc[0], net_df['date_time'].iloc[-1])
    cols = ['symbol', 'param', 'tot_ret', 'ann_ret', 'sharpe',
            'max_drawdown', 'alpha', 'beta', 'ret_ratio', 'ir', 'hold_k',
            'position', 'trade_times', 'win_r', 'profit_r', 'long_return',
            'short_return', 'start_time', 'end_time']

    if plot_in_loops:
        param_str = (symbol.replace('/', '_') +
